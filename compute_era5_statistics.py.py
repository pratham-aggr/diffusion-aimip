...
def compute_statistics(zarr_path: str, output_dir: Path, train_period: slice):
    """
    ...
    if zarr_path.startswith("gs://") and HAS_GCSFS:
        ...
        ds = xr.open_zarr(store, decode_times=True, chunks={"time": 100})
    else:
        # For local paths, use standard open_zarr
    ds = xr.open_zarr(zarr_path, decode_times=True, chunks={"time": 100})
...
if __name__ == "__main__":
    ...
    compute_statistics(zarr_path, output_dir, train_period)

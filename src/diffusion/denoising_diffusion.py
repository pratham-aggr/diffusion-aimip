...
class GaussianDiffusion(BaseDiffusion):
    ...
    def model_predictions(self, x, t, condition=None, clip_x_start=False, **kwargs):
        model_output = self.model(x, time=t, condition=condition, **kwargs)
    ...
    def p_mean_variance(self, x, t, x_self_cond=None, clip_denoised=False, **kwargs):
        preds = self.model_predictions(x, t, x_self_cond, clip_x_start=clip_denoised, **kwargs)
    ...
    def p_sample(self, x, batched_times: Tensor, t: int, x_self_cond=None, clip_denoised=False, **kwargs):
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(
            x=x, t=batched_times, x_self_cond=x_self_cond, clip_denoised=clip_denoised, **kwargs
        )
    ...
    @torch.inference_mode()
    def p_sample_loop(self, condition, shape, log_every_t=None, verbose: bool = False, **kwargs):
        ...
        for t in tsteps_r:
            ...
            img, x_start = self.p_sample(img, batched_times, x_self_cond=condition, clip_denoised=False, t=t, **kwargs)
    ...
    def sample(self, condition=None, **kwargs):
        ...
        return sample_fn(
            condition, shape=(batch_size, channels, self.spatial_shape_out[0], self.spatial_shape_out[1]), **kwargs
        )
    ...
    def p_losses(self, x_start, condition, t, noise=None, **kwargs):
        """
        ...
        model_preds = self.model(x_t, time=t, condition=condition, **kwargs)

...
class DDPM(BaseDiffusion):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # log.info(f"{self.__class__.__name__}: Running in {self.parameterization}-prediction mode")
        ...
        if self.use_ema:
            ...
            # log.info(f"Keeping EMAs of {len(list(self.model_ema.buffers()))}.")
    ...
    def p_mean_variance(self, x, condition, t, clip_denoised: bool = False, **kwargs):
        model_out = self.model(x, time=t, condition=condition, **kwargs)
        if self.parameterization == "pred_noise":
    ...
    def p_sample(self, x, condition, t, clip_denoised=False, repeat_noise=False, **kwargs):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(
            x=x, condition=condition, t=t, clip_denoised=clip_denoised, **kwargs
        )
    ...
    def sample(self, condition, batch_size: int = None, return_intermediates=False, **kwargs):
        ...
        preds, intermediates = self.p_sample_loop(
            condition=condition,
            shape=(batch_size, self.channels, self.spatial_shape_out[0], self.spatial_shape_out[1]),
            **kwargs,
        )
    ...
    def p_losses(self, x_start, condition, t, noise=None, **kwargs):
        ...
        model_out = self.model(x_noisy, condition=condition, time=t, **kwargs)

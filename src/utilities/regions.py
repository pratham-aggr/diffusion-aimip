...
class Region:
    ...
    def apply(self, dataset: xr.Dataset, weights: xr.DataArray) -> tuple[xr.Dataset, xr.Dataset]:
...
class SliceRegion(Region):
    ...
    lat_slice: t.Optional[t.Union[slice, list[slice]]] = dataclasses.field(default_factory=lambda: slice(None, None))
    lon_slice: t.Optional[t.Union[slice, list[slice]]] = dataclasses.field(default_factory=lambda: slice(None, None))
    ...
    def apply(  # pytype: disable=signature-mismatch
        ...
        lats = self.lat_slice if isinstance(self.lat_slice, list) else [self.lat_slice]
        lons = self.lon_slice if isinstance(self.lon_slice, list) else [self.lon_slice]

        lats = xr.concat([dataset.latitude.sel(latitude=s) for s in lats], dim="latitude")
        lons = xr.concat([dataset.longitude.sel(longitude=s) for s in lons], dim="longitude")

...
from ...utilities.utils import exists
...
class Unet(BaseModel):
    ...
    Args:
        ...
        dim (int): Number of channels in the hidden layers
        activation_function (str): Activation function to use
        norm (bool): Whether to use normalization
        dim_mults (list): List of channel multipliers for each resolution
    ...
    def __init__(
        ...
        activation_function,
        norm: bool = True,
        dim_mults: Union[Tuple[int, ...], List[int]] = (1, 2, 2, 4),
        ...
        upsample_dims: tuple = None,  # (256, 256) or (128, 128) etc.
        **kwargs,
    ) -> None:
        super().__init__(**kwargs)
        assert with_time_emb, "Currently, time embedding is required"
        self.hidden_channels = dim
        self.activation = activation_function
        self.param_conditioning = param_conditioning
        self.activation: nn.Module = ACTIVATION_REGISTRY.get(activation_function, None)
        if self.activation is None:
            raise NotImplementedError(f"Activation {activation_function} not implemented")
        ...
        insize = self.num_input_channels + self.num_conditional_channels
        n_channels = dim
        ...
        for i in range(n_resolutions):
            ...
            out_channels = in_channels * dim_mults[i]
            # Add `n_blocks`
    ...
    def forward(self, x: torch.Tensor, time, z=None, condition=None):
        ...
        # Not necessary here for this code base, as it is handled by the module
        # assert x.dim() == 5
        orig_shape = x.shape
        ...
        if exists(self.upsampler):
            x = F.interpolate(x, size=orig_shape[-2:], mode=self.hparams.outer_sample_mode)
...
class FourierUnet(nn.Module):
    ...
    def __init__(
        ...
        insize = self.ntime_history * (self.n_input_scalar_components + self.n_input_vector_components * 2)

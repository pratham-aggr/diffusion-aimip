...
from modulus.utils.sfno.distributed.mappings import (
    ...
    gather_from_spatial_parallel_region,
    reduce_from_matmul_parallel_region,
    scatter_to_matmul_parallel_region,
    scatter_to_spatial_parallel_region,
)
...
class DistributedPatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        ) % spatial_comm_size == 0, "Error, make sure that the spatial comm size evenly divides patched W"
...
class DistributedAFNO2Dv2(nn.Module):
    ...
    def forward(self, x):  # pragma: no cover
        ...
        o2[:, :, :, total_modes - kept_modes : total_modes + kept_modes, :kept_modes, :] = self.mult_handle(
            o1, self.w2, self.b2
        )

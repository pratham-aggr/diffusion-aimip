...
class SpectralConv2d(nn.Module):
    ...
    def __init__(
        ...
        self.w = nn.Parameter(
            self.scale * torch.randn(self.hidden_size, self.hidden_size, self.modes_lat, self.modes_lon, 2)
        )
        # optional bias
        if bias:
...
class SpectralAttention2d(nn.Module):
    ...
    def __init__(
        ...
        if bias:
            self.b = nn.ParameterList(
                [self.scale * torch.randn(self.hidden_size, 1, 2) for _ in range(self.spectral_layers)]
            )
    ...
    def forward_mlp(self, xr):  # pragma: no cover
        ...
        for layer in range(self.spectral_layers):
            if hasattr(self, "b"):
                xr = self.mul_add_handle(xr, self.w[layer].to(xr.dtype), self.b[layer].to(xr.dtype))
            else:
                xr = self.mul_handle(xr, self.w[layer].to(xr.dtype))
...
class SpectralAttentionS2(nn.Module):
    ...
    def __init__(
        ...
        w = [self.scale * torch.randn(self.embed_dim, self.hidden_size, 2)]
        # w = [self.scale * torch.randn(self.embed_dim + 4*self.embed_freqs, self.hidden_size, 2)]
        for layer in range(1, self.spectral_layers):
            w.append(self.scale * torch.randn(self.hidden_size, self.hidden_size, 2))
        self.w = nn.ParameterList(w)

        if bias:
            self.b = nn.ParameterList(
                [self.scale * torch.randn(2 * self.hidden_size, 1, 1, 2) for _ in range(self.spectral_layers)]
            )

    ...
    def __init__(
        ...
        **kwargs,
        ...
        if with_time_emb:
            # time embeddings
            self.time_dim = dim * 2
            self.time_emb_mlp = get_time_embedder(self.time_dim, dim, sinusoidal_embedding="true")
        else:
            self.time_dim = None
        ...
        self.input_ops = torch.nn.ModuleList(
            [
                UNetBlock(dim, dim * 2, transposed=False, bn=True, relu=False, size=base_kernel_size, **block_kwargs),
                UNetBlock(
                    dim * 2, dim * 2, transposed=False, bn=True, relu=False, size=base_kernel_size, **block_kwargs
                ),
                UNetBlock(
                    dim * 2, dim * 4, transposed=False, bn=True, relu=False, size=base_kernel_size, **block_kwargs
                ),
        ...
        self.output_ops = torch.nn.ModuleList(
            [
                ...
                UNetBlock(dim * 8 * 2, dim * 4, transposed=True, bn=True, relu=True, size=4, **block_kwargs),
                UNetBlock(
                    dim * 4 * 2, dim * 2, transposed=True, bn=True, relu=True, size=base_kernel_size, **block_kwargs
                ),
                UNetBlock(
                    dim * 2 * 2, dim * 2, transposed=True, bn=True, relu=True, size=base_kernel_size, **block_kwargs
                ),
                UNetBlock(
                    dim * 2 * 2, dim, transposed=True, bn=True, relu=True, size=base_kernel_size, **block_kwargs
                ),
            ]
        )

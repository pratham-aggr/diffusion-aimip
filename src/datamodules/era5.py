...
class ERA5DataModuleBase(BaseDataModule):
    def __init__(
        ...
        possible_initial_times_eval: Optional[str] = None,
        ...
        # ClimateBench compatibility params (ignored for ERA5)
    ...
    def get_split_dataset(self, split: str, time_slice: slice, **kwargs) -> ERA5DatasetBase:
        ...
        if self.hparams.use_dask:
            ...
            chunks = None
        log.info(f"Opening zarr file: {self.zarr_path} (this may take a while for large files...)")
        # Use consolidated=True for faster metadata access (reads single .zmetadata file instead of many)
        # consolidated=False can cause hangs on large zarr datasets
        ds = xr.open_zarr(self.zarr_path, decode_times=True, chunks=chunks, mask_and_scale=False, consolidated=True)
        log.info(f"Successfully opened zarr file. Dataset shape: {dict(ds.dims)}")
        # Select time slice FIRST to reduce dataset size before any computations
        log.info(f"Selecting time slice: {time_slice}")
        ds = ds.sel(time=time_slice)
        log.info(f"Time slice selected. New shape: {dict(ds.dims)}")
        # Convert geopotential from m²/s² to meters (geopotential height) for CMIP compliance
        # Do this AFTER time slicing to avoid computing on entire dataset
        if "geopotential" in ds.data_vars:
            log.info("Converting geopotential from m²/s² to meters (on sliced dataset)...")
            g = 9.80665  # standard gravity [m/s^2]
            ds["geopotential"] = ds["geopotential"] / g
            log.info("Geopotential conversion completed")
        ...
        log.info(f"Calling _get_split_dataset for split={split} with kwargs keys: {list(kwargs.keys())}")
        dset = self._get_split_dataset(ds, split, **kwargs)
        log.info(f"_get_split_dataset completed. Dataset type: {type(dset)}")
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        log.info(f"Starting datamodule setup for stage: {stage}")
        # Set the correct tensor datasets for the train, val, and test sets
        ds_splits = dict()
        if stage in ["fit", "validate", None]:
            log.info(f"Loading training dataset with slice: {self.train_slice}")
    ...
    def get_epoch_aggregators(
        ...
        for mask, mask_name in zip(masks, mask_names):
            ...
            # Filter snapshot variables to only include those that exist in output_vars
            all_output_vars = set(self.hparams.output_vars)
            candidate_snapshot_vars = ["temperature_850", "2m_temperature", "10m_u_component_of_wind", "geopotential_500", "surface_pressure"]
            snapshot_var_names = [vr for vr in candidate_snapshot_vars if vr in all_output_vars]
            snapshot_var_names = [f"{vr}_normed" for vr in snapshot_var_names if vr != "geopotential_500"]
            if "geopotential_500" in all_output_vars:
                snapshot_var_names += ["geopotential_500"]
            snapshot_kwargs = {
            ...
            # Filter spectra variables to only include those that exist in output_vars
            candidate_spectra_surface = ["2m_temperature", "10m_u_component_of_wind", "mean_sea_level_pressure"]
            spectra_names = [vr for vr in candidate_spectra_surface if vr in all_output_vars]
            spectra_levels = [50, 100, 500, 700, 850, 1000]
            for base_var in ["temperature", "geopotential", "u_component_of_wind", "v_component_of_wind", "specific_humidity"]:
                for lev in spectra_levels:
                    var_name = f"{base_var}_{lev}"
                    if var_name in all_output_vars:
                        spectra_names.append(var_name)
            spectra_kwargs = {
        ...
        if save_to_path is not None:
            ...
            aggregators_all["save_to_disk"] = SaveToDiskAggregator(
                ...
                var_names=self.hparams.output_vars,  # Add var_names for dictionary-based target_data
            )
...
class ERA5Dataset2D(ERA5DatasetBase):
    def __init__(
        ...
        **kwargs,
        ...
        self.possible_2d_vars = [
            ...
            "total_precipitation_6hr",  # 6-hourly accumulated precipitation
            "total_precipitation_24hr",  # 24-hourly accumulated precipitation
        ]
        possible_3d_vars = [
            "geopotential",
            "specific_humidity",
        ...
        if preselect_vars:
            ...
            # Only select levels if we have 3D variables with pressure levels
            if self.all_levels and 'level' in ds.dims:
    ...
    def loss_weights_tensor(self) -> Optional[torch.Tensor]:
        ...
        # Skip if both pressure weighting and surface vars weighting are disabled
        if self.loss_pressure_weighting in [False, None] and self.loss_surface_vars_weighting is None:
            return weights
            
        if self.loss_pressure_weighting is not None and self.loss_pressure_weighting not in [False, None]:
            # Throw error only if pressure weighting is enabled but levels are not specified
            if self.loss_pressure_weighting_levels is None:
                raise ValueError(
                    ...
                    f"\nWhen loss_pressure_weighting is enabled, loss_pressure_weighting_levels must be specified."
                )

        var_to_weight = torch.ones(len(self.output_vars))
        if self.loss_pressure_weighting and self.loss_pressure_weighting not in [False, None]:
                if self.loss_pressure_weighting == "graphcast":
                    ...
                    pass
                elif self.loss_pressure_weighting in [False, None]:
                    pass  # No pressure weighting, skip
                else:
    ...
    def __getitem__(self, idx):
        ...
        if self.preselect_vars:
            if self.preprocess_to_tensor:
                ...
                arrays["dynamics"] = {vr: torch.from_numpy(self.var_to_ds[vr].isel(time=idx_slice_xr).values) for vr in self.all_vars}
        # ------------------ Focus on this part! ------------------
        else:
            try:
            ...
            if len(self.input_only_vars) > 0:
                # For emulation: separate inputs from outputs
                input_vars_dict = {vr: dynamics.pop(vr) for vr in self.input_only_vars}
                output_vars_dict = dynamics  # Remaining are outputs
                
                # Normalize
                input_vars_normalized = self.normalizer.normalize(input_vars_dict)
                output_vars_normalized = self.normalizer.normalize(output_vars_dict)
                
                # Replace NaN values with 0 in inputs (e.g., SST over land)
                # This is necessary for variables like sea_surface_temperature which are only defined over ocean
                for vr in self.input_only_vars:
                    if torch.isnan(input_vars_normalized[vr]).any():
                        input_vars_normalized[vr] = torch.nan_to_num(input_vars_normalized[vr], nan=0.0)
                
                # Pack into tensors for batching
                # Emulation module's pack_data will detect these are already tensors and skip re-packing
                arrays["inputs"] = self.in_only_packer.pack(input_vars_normalized)
                arrays["targets"] = self.out_packer.pack(output_vars_normalized)
                # raw_targets will be computed on-the-fly in emulation module by unpacking/denormalizing targets
            else:
                # Standard forecasting: same variables for input and output

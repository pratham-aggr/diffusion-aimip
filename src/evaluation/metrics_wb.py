...
from src.utilities.regions import Region
...
# From GraphCast below. It is equivalent to get_lat_weights (see 2024-05-29-analyze-latitude-loss-weights notebook)
...
class Metric:
    ...
    def compute(
        ...
        if "time" in forecast.dims:
            ...
            raise ValueError(f"Forecast has neither valid_time or init_time dimension {forecast}")
        return self.compute_chunk(forecast, truth, region=region).mean(avg_dim)


def _spatial_average(dataset: xr.Dataset, region: t.Optional[Region] = None, skipna: bool = False) -> xr.Dataset:
    ...
    def compute_chunk(
        ...
        if self.wind_vector_rmse is not None:
            for wv in self.wind_vector_rmse:
                results[wv.vector_name] = wv.compute_chunk(forecast, truth, region=region)
        return results
...
class ACC(Metric):
    ...
    def compute_chunk(
        ...
        try:
            ...
            climatology_chunk = self.climatology[list(clim_var_dict.keys())].rename(clim_var_dict)
        ...
        return _spatial_average(forecast_anom * truth_anom, region=region) / np.sqrt(
            _spatial_average(forecast_anom**2, region=region) * _spatial_average(truth_anom**2, region=region)
        )
...
class SpatialSEEPS(Metric):
    ...
    def compute_chunk(
        ...
        out = (
            forecast_cat.rename({"seeps_cat": "forecast_cat"}) * truth_cat.rename({"seeps_cat": "truth_cat"})
        ).compute()
...
def _get_n_ensemble(
    ...
    if n_ensemble < expect_n_ensemble_at_least:
        raise ValueError(f"{n_ensemble=} is less than expected size of " f"{expect_n_ensemble_at_least}")
    return n_ensemble
...
class EnsembleMetric(Metric):
    ...
    def _ensemble_slice(self, ds: xr.Dataset, slice_obj: slice) -> xr.Dataset:
        ...
        return ds.assign_coords({self.ensemble_dim: np.arange(ds.dims[self.ensemble_dim])})
...
class CRPS(EnsembleMetric):
    ...
    def compute_chunk(
        ...
        return CRPSSkill(self.ensemble_dim).compute_chunk(forecast, truth, region=region) - 0.5 * CRPSSpread(
            self.ensemble_dim
...
def _pointwise_crps_spread(forecast: xr.Dataset, truth: xr.Dataset, ensemble_dim: str) -> xr.Dataset:
    ...
    return 2 * (((2 * rank - n_ensemble - 1) * forecast).mean(ensemble_dim, skipna=False)) / (n_ensemble - 1)


def _pointwise_crps_skill(forecast: xr.Dataset, truth: xr.Dataset, ensemble_dim: str) -> xr.Dataset:
...
class EnsembleStddev(EnsembleMetric):
    ...
    def compute_chunk(
        ...
        if n_ensemble == 1:
            ...
            return _spatial_average_l2_norm(forecast.std(self.ensemble_dim, ddof=1), region=region)
...
class EnsembleMeanRMSE(EnsembleMetric):
    ...
    def compute_chunk(
        ...
        return _spatial_average_l2_norm(truth - forecast.mean(self.ensemble_dim), region=region)
...
class EnergyScore(EnsembleMetric):
    ...
    def compute_chunk(
        ...
        ) - 0.5 * EnergyScoreSpread(self.ensemble_dim).compute_chunk(forecast, truth, region=region)
    ...
    def compute_chunk(
        ...
        if n_ensemble == 1:
            ...
            return _spatial_average_l2_norm(
                self._ensemble_slice(forecast, slice(None, -1)) - self._ensemble_slice(forecast, slice(1, None)),

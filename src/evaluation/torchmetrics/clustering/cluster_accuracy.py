...
from src.evaluation.torchmetrics.functional.clustering.cluster_accuracy import _cluster_accuracy_compute
...
class ClusterAccuracy(Metric):
    ...
    Raises:
        RuntimeError:
            If ``torch_linear_assignment`` is not installed. To install, run ``pip install src.evaluation.torchmetrics[clustering]``.
        ValueError
            If ``num_classes`` is not a positive integer

    Example::
        >>> import torch
        >>> from src.evaluation.torchmetrics.clustering import ClusterAccuracy
    ...
    def __init__(self, num_classes: int, **kwargs: Any) -> None:
        ...
        if not _TORCH_LINEAR_ASSIGNMENT_AVAILABLE:
            raise RuntimeError(
                "Missing `torch_linear_assignment`. Please install it with `pip install src.evaluation.torchmetrics[clustering]`."
            )

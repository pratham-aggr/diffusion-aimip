...
from src.evaluation.torchmetrics.functional.classification.accuracy import _accuracy_reduce
...
class MultilabelAccuracy(MultilabelStatScores):
    ...
    Example (preds is int tensor):
        >>> from torch import tensor
        >>> from src.evaluation.torchmetrics.classification import MultilabelAccuracy
        >>> target = tensor([[0, 1, 0], [1, 0, 1]])
        >>> preds = tensor([[0, 0, 1], [1, 0, 1]])
...
class Accuracy(_ClassificationTaskWrapper):
    ...
    :class:`~src.evaluation.torchmetrics.classification.BinaryAccuracy`, :class:`~src.evaluation.torchmetrics.classification.MulticlassAccuracy` and
    :class:`~src.evaluation.torchmetrics.classification.MultilabelAccuracy` for the specific details of each argument influence and
    examples.

...
class BinaryPrecision(BinaryStatScores):
    ...
    Example (preds is int tensor):
        >>> from torch import tensor
        >>> from src.evaluation.torchmetrics.classification import BinaryPrecision
...
class MulticlassPrecision(MulticlassStatScores):
    ...
    Example (preds is int tensor):
        >>> from torch import tensor
        >>> from src.evaluation.torchmetrics.classification import MulticlassPrecision
...
class MultilabelPrecision(MultilabelStatScores):
    ...
    Example (preds is int tensor):
        >>> from torch import tensor
        >>> from src.evaluation.torchmetrics.classification import MultilabelPrecision
        >>> target = tensor([[0, 1, 0], [1, 0, 1]])
        >>> preds = tensor([[0, 0, 1], [1, 0, 1]])
...
class BinaryRecall(BinaryStatScores):
    ...
    Example (preds is int tensor):
        >>> from torch import tensor
        >>> from src.evaluation.torchmetrics.classification import BinaryRecall
        >>> target = tensor([0, 1, 0, 1, 0, 1])
        >>> preds = tensor([0, 0, 1, 1, 0, 1])
        >>> metric = BinaryRecall()
        >>> metric(preds, target)
...
class MulticlassRecall(MulticlassStatScores):
    ...
    Example (preds is int tensor):
        >>> from torch import tensor
        >>> from src.evaluation.torchmetrics.classification import MulticlassRecall
        >>> target = tensor([2, 1, 0, 0])
        >>> preds = tensor([2, 1, 0, 1])
...
class MultilabelRecall(MultilabelStatScores):
    ...
    Example (preds is int tensor):
        >>> from torch import tensor
        >>> from src.evaluation.torchmetrics.classification import MultilabelRecall
        >>> target = tensor([[0, 1, 0], [1, 0, 1]])
        >>> preds = tensor([[0, 0, 1], [1, 0, 1]])
...
class Precision(_ClassificationTaskWrapper):
    ...
    :class:`~src.evaluation.torchmetrics.classification.BinaryPrecision`, :class:`~src.evaluation.torchmetrics.classification.MulticlassPrecision` and
    :class:`~src.evaluation.torchmetrics.classification.MultilabelPrecision` for the specific details of each argument influence and
    examples.
...
class Recall(_ClassificationTaskWrapper):
    ...
    This function is a simple wrapper to get the task specific versions of this metric, which is done by setting the
    ``task`` argument to either ``'binary'``, ``'multiclass'`` or ``multilabel``. See the documentation of
    :class:`~src.evaluation.torchmetrics.classification.BinaryRecall`,
    :class:`~src.evaluation.torchmetrics.classification.MulticlassRecall` and :class:`~src.evaluation.torchmetrics.classification.MultilabelRecall`

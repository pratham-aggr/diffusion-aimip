...
class NoTrainInceptionV3(_FeatureExtractorInceptionV3):
    ...
    def _torch_fidelity_forward(self, x: Tensor) -> tuple[Tensor, ...]:
        ...
        """
        vassert(
            torch.is_tensor(x) and x.dtype == torch.uint8, "Expecting image as torch.Tensor with dtype=torch.uint8"
        )
        features = {}
...
class FrechetInceptionDistance(Metric):
    ...
    .. hint::
        ...
        is installed. Either install as ``pip install src.evaluation.torchmetrics[image]`` or ``pip install torch-fidelity``

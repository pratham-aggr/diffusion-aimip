...
from src.evaluation.torchmetrics.image.fid import NoTrainInceptionV3, _compute_fid
from src.evaluation.torchmetrics.metric import Metric
from src.evaluation.torchmetrics.utilities.data import dim_zero_cat
from src.evaluation.torchmetrics.utilities.imports import _MATPLOTLIB_AVAILABLE, _TORCH_FIDELITY_AVAILABLE
...
class MemorizationInformedFrechetInceptionDistance(Metric):
    ...
    .. hint::
        ...
        src.evaluation.torchmetrics[image]`` or ``pip install scipy``

    .. hint::
        Using this metric with the default feature extractor requires that ``torch-fidelity``
        is installed. Either install as ``pip install src.evaluation.torchmetrics[image]`` or
        ...
        if isinstance(feature, int):
            if not _TORCH_FIDELITY_AVAILABLE:
                raise ModuleNotFoundError(
                    ...
                    " Either install as `pip install src.evaluation.torchmetrics[image]` or `pip install torch-fidelity`."
                )

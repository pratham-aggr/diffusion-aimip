...
from src.evaluation.torchmetrics.functional.regression.mae import (
...
class MeanAbsoluteError(Metric):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # self.add_state("sum_abs_error", default=torch.zeros(error_shape), dist_reduce_fx="sum")
        self.add_state("total", default=tensor(0), dist_reduce_fx="sum")

    def update(self, preds: Tensor, target: Tensor) -> None:
        """Update state with predictions and targets."""
        sum_abs_error, num_obs = _mean_absolute_error_update(preds, target, flatten=self.weights is None)

        self.sum_abs_error = sum_abs_error if self.total == 0 else self.sum_abs_error + sum_abs_error
        self.total += num_obs
    ...
    def compute(self) -> Tensor:
        ...
        if self.weights is not None:
            sum_abs_error = weighted_mean(sum_abs_error, weights=self.weights)
        return _mean_absolute_error_compute(sum_abs_error, self.total)

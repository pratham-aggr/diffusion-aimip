...
from src.evaluation.torchmetrics.functional.regression.mse import (
...
class MeanSquaredError(Metric):
    ...
    def update(self, preds: Tensor, target: Tensor) -> None:
        ...
        sum_squared_error, num_obs = _mean_squared_error_update(preds, target, flatten=self.weights is None)
        self.sum_squared_error = sum_squared_error if self.total == 0 else self.sum_squared_error + sum_squared_error
        self.total += num_obs
    ...
    def compute(self) -> Tensor:
        ...
        if self.weights is not None:
            sum_squared_error = weighted_mean(sum_squared_error, weights=self.weights)
        return _mean_squared_error_compute(sum_squared_error, self.total, squared=self.squared)

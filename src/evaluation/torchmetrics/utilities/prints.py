...
from src.evaluation.torchmetrics import _logger as log
...
def _deprecated_root_import_class(name: str, domain: str) -> None:
    ...
    _future_warning(
        f"Importing `{name}` from `src.evaluation.torchmetrics` was deprecated and will be removed in 2.0."
        f" Import `{name}` from `src.evaluation.torchmetrics.{domain}` instead."
    )
...
def _deprecated_root_import_func(name: str, domain: str) -> None:
    ...
    _future_warning(
        f"Importing `{name}` from `src.evaluation.torchmetrics.functional` was deprecated and will be removed in 2.0."
        f" Import `{name}` from `src.evaluation.torchmetrics.{domain}` instead."
    )

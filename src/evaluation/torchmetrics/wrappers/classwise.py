...
from src.evaluation.torchmetrics.wrappers.abstract import WrapperMetric
...
class ClasswiseWrapper(WrapperMetric):
    ...
    Example::
        ...
        >>> from src.evaluation.torchmetrics.wrappers import ClasswiseWrapper
    ...
    Example::
        Classwise can also be used in combination with :class:`~src.evaluation.torchmetrics.MetricCollection`. In this case, everything
        will be flattened into a single dictionary:

        >>> from torch import randint, randn
        >>> from src.evaluation.torchmetrics import MetricCollection
        >>> from src.evaluation.torchmetrics.wrappers import ClasswiseWrapper
        >>> from src.evaluation.torchmetrics.classification import MulticlassAccuracy, MulticlassRecall
        >>> labels = ["horse", "fish", "dog"]
    ...
    def __init__(
        ...
        if not isinstance(metric, Metric):
            raise ValueError(
                f"Expected argument `metric` to be an instance of `src.evaluation.torchmetrics.Metric` but got {metric}"
            )
        self.metric = metric

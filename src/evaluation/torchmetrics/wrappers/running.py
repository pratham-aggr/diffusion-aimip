...
class Running(WrapperMetric):
    ...
    Example (single metric):
        >>> from torch import tensor
        >>> from src.evaluation.torchmetrics.wrappers import Running
    ...
    Example (metric collection):
        ...
        >>> from src.evaluation.torchmetrics.aggregation import SumMetric, MeanMetric
    ...
    def __init__(self, base_metric: Metric, window: int = 5) -> None:
        ...
        if not isinstance(base_metric, Metric):
            raise ValueError(
                f"Expected argument `metric` to be an instance of `src.evaluation.torchmetrics.Metric` but got {base_metric}"
            )

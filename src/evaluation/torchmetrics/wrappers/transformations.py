...
class MetricInputTransformer(WrapperMetric):
    ...
    def __init__(self, wrapped_metric: Union[Metric, MetricCollection], **kwargs: dict[str, Any]) -> None:
        ...
        if not isinstance(wrapped_metric, (Metric, MetricCollection)):
            raise TypeError(
                f"Expected wrapped metric to be an instance of `src.evaluation.torchmetrics.Metric` or "
                f"`src.evaluation.torchmetrics.MetricsCollection`but received {wrapped_metric}"
            )
...
class LambdaInputTransformer(MetricInputTransformer):
    ...
    Example:
        ...
        >>> from src.evaluation.torchmetrics.wrappers import LambdaInputTransformer
        >>>
...
class BinaryTargetTransformer(MetricInputTransformer):
    ...
    Example:
        >>> import torch
        >>> from src.evaluation.torchmetrics.retrieval import RetrievalMRR
        >>> from src.evaluation.torchmetrics.wrappers import BinaryTargetTransformer
        >>>
        >>> preds = torch.tensor([0.9, 0.8, 0.7, 0.6, 0.5, 0.6, 0.7, 0.8, 0.5, 0.4])

...
from src.evaluation.torchmetrics.utilities.checks import _check_retrieval_functional_inputs
...
def retrieval_average_precision(preds: Tensor, target: Tensor, top_k: Optional[int] = None) -> Tensor:
    ...
    Example:
        >>> from src.evaluation.torchmetrics.functional.retrieval import retrieval_average_precision
    ...
    return torch.div(
        (torch.arange(len(positions), device=positions.device, dtype=torch.float32) + 1), positions
    ).mean()

...
def _mean_squared_error_update(preds: Tensor, target: Tensor, flatten: bool = False) -> tuple[Tensor, int]:
...
def _mean_squared_error_compute(
    ...
    """
    sum_squared_error = sum_squared_error / num_obs
    return sum_squared_error if squared else torch.sqrt(sum_squared_error)
...
def mean_squared_error(preds: Tensor, target: Tensor, squared: bool = True, num_outputs: int = 1) -> Tensor:
    ...
    Example:
        >>> from src.evaluation.torchmetrics.functional.regression import mean_squared_error
    ...
    """
    sum_squared_error, num_obs = _mean_squared_error_update(preds, target, flatten=num_outputs == 1)

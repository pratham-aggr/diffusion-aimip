...
from src.evaluation.torchmetrics.utilities.distributed import reduce
    ...
    if pan_lr is None:
        if not _TORCHVISION_AVAILABLE:
            raise ValueError(
                ...
                "installed. Please install with `pip install torchvision` or `pip install src.evaluation.torchmetrics[image]`."
            )
        from torchvision.transforms.functional import resize

        from src.evaluation.torchmetrics.functional.image.utils import _uniform_filter
...
def spatial_distortion_index(
    ...
    Example:
        >>> from torch import rand
        >>> from src.evaluation.torchmetrics.functional.image import spatial_distortion_index
        >>> preds = rand([16, 3, 32, 32])
        >>> ms = rand([16, 3, 16, 16])
        >>> pan = rand([16, 3, 32, 32])

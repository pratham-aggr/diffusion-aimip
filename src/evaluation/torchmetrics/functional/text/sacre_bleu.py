...
from src.evaluation.torchmetrics.functional.text.bleu import _bleu_score_compute, _bleu_score_update
...
_FLORES_LOCAL_DIR = os.path.join(tempfile.gettempdir(), "src.evaluation.torchmetrics-flores")
    ...
    def _check_tokenizers_validity(cls: type["_SacreBLEUTokenizer"], tokenize: _TokenizersLiteral) -> None:
        ...
        if tokenize == "intl" and not _REGEX_AVAILABLE:
            raise ModuleNotFoundError(
                ...
                " Use `pip install regex` or `pip install src.evaluation.torchmetrics[text]`."
            )
        ...
        if tokenize == "ja-mecab" and not (_MECAB_AVAILABLE and _IPADIC_AVAILABLE):
            raise ModuleNotFoundError(
                ...
                " Use `pip install mecab-python3 ipadic` or `pip install src.evaluation.torchmetrics[text]`."
            )
        ...
        if tokenize == "ko-mecab" and not (_MECAB_KO_AVAILABLE and _MECAB_KO_DIC_AVAILABLE):
            raise ModuleNotFoundError(
                ...
                " Use `pip install mecab_ko mecab_ko_dic` or `pip install src.evaluation.torchmetrics[text]`."
            )
        ...
        if "flores" in tokenize and not _SENTENCEPIECE_AVAILABLE:
            raise ModuleNotFoundError(
                ...
                " Use `pip install sentencepiece` or `pip install src.evaluation.torchmetrics[text]`."
            )
...
def sacre_bleu_score(
    ...
    Example:
        >>> from src.evaluation.torchmetrics.functional.text import sacre_bleu_score

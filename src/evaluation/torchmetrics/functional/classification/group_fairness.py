...
from src.evaluation.torchmetrics.utilities.data import _flexible_bincount
...
def binary_groups_stat_rates(
    ...
    Example (preds is int tensor):
        >>> from src.evaluation.torchmetrics.functional.classification import binary_groups_stat_rates
...
def demographic_parity(
    ...
    Example (preds is int tensor):
        >>> from src.evaluation.torchmetrics.functional.classification import demographic_parity
        >>> preds = torch.tensor([0, 1, 0, 1, 0, 1])
        >>> groups = torch.tensor([0, 1, 0, 1, 0, 1])
...
def equal_opportunity(
    ...
    Example (preds is int tensor):
        >>> from src.evaluation.torchmetrics.functional.classification import equal_opportunity
        >>> target = torch.tensor([0, 1, 0, 1, 0, 1])
        >>> preds = torch.tensor([0, 1, 0, 1, 0, 1])
        >>> groups = torch.tensor([0, 1, 0, 1, 0, 1])
...
def binary_fairness(
    ...
    :func:`~src.evaluation.torchmetrics.functional.classification.demographic_parity`
    and :func:`~src.evaluation.torchmetrics.functional.classification.equal_opportunity` for the specific details of

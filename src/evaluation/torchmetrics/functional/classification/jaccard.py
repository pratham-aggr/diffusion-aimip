...
def _jaccard_index_reduce(
    ...
    if average == "binary":
        return _safe_divide(
            confmat[1, 1], (confmat[0, 1] + confmat[1, 0] + confmat[1, 1]), zero_division=zero_division
        )
...
def binary_jaccard_index(
    ...
    Example (preds is int tensor):
        >>> from torch import tensor
        >>> from src.evaluation.torchmetrics.functional.classification import binary_jaccard_index
...
def multiclass_jaccard_index(
    ...
    Example (pred is integer tensor):
        >>> from torch import tensor
        >>> from src.evaluation.torchmetrics.functional.classification import multiclass_jaccard_index
...
def multilabel_jaccard_index(
    ...
    Example (preds is int tensor):
        >>> from torch import tensor
        >>> from src.evaluation.torchmetrics.functional.classification import multilabel_jaccard_index
...
def jaccard_index(
    ...
    :func:`~src.evaluation.torchmetrics.functional.classification.binary_jaccard_index`,
    :func:`~src.evaluation.torchmetrics.functional.classification.multiclass_jaccard_index` and
    :func:`~src.evaluation.torchmetrics.functional.classification.multilabel_jaccard_index` for
    ...
    if task == ClassificationTask.MULTICLASS:
        ...
        return multiclass_jaccard_index(
            preds, target, num_classes, average, ignore_index, validate_args, zero_division
        )

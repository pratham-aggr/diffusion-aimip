...
from src.ace_inference.core import get_device
...
def test_zonal_mean_lat_varying():
    ...
    agg.record_batch(loss, {"a": arr}, {"a": arr}, {"a": arr}, {"a": arr}, i_time_start=0)
    for data in (agg._target_data, agg._gen_data):
...
def test_zonal_mean_mulitple_time_slices():
    ...
    for i_time in range(0, n_time, n_time_in_memory):
        ...
        agg.record_batch(loss, {"a": arr}, {"a": arr}, {"a": arr}, {"a": arr}, i_time_start=i_time)
    for data in (agg._target_data, agg._gen_data):
        torch.testing.assert_close(

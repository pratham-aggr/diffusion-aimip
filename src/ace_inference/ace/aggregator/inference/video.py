...
class _ErrorVideoData:
    ...
    def record_batch(
        ...
        if self._min_err_data is None:
            self._min_err_data = _initialize_video_from_batch(gen_data, self._n_timesteps, fill_value=np.inf)
        if self._max_err_data is None:
            self._max_err_data = _initialize_video_from_batch(gen_data, self._n_timesteps, fill_value=-np.inf)
    ...
    def get(
        ...
        if self._mse_data is None or self._min_err_data is None or self._max_err_data is None:
...
class _MeanVideoData:
    ...
    def record_batch(
        ...
        if self._target_data is None:
            self._target_data = _initialize_video_from_batch(target_data, self._n_timesteps)
...
class _VarianceVideoData:
    ...
    def record_batch(
        ...
        if self._target_means is None:
            self._target_means = _initialize_video_from_batch(target_data, self._n_timesteps)
        ...
        if self._target_squares is None:
            self._target_squares = _initialize_video_from_batch(target_data, self._n_timesteps)

        if self._gen_squares is None:
            self._gen_squares = _initialize_video_from_batch(gen_data, self._n_timesteps)
...
def _initialize_video_from_batch(batch: Mapping[str, torch.Tensor], n_timesteps: int, fill_value: float = 0.0):
    """
...
class VideoAggregator:
    ...
    def __init__(
        ...
        if enable_extended_videos:
            self._error_data: Optional[_ErrorVideoData] = _ErrorVideoData(n_timesteps=n_timesteps, dist=dist)
            self._variance_data: Optional[_VarianceVideoData] = _VarianceVideoData(n_timesteps=n_timesteps, dist=dist)
        ...
        if self._variance_data is not None:
            ...
            for name in gen_data:
                video_data[f"{label}/gen_var/{name}"] = _MaybePairedVideoData(
                    caption=(f"Variance of gen data for {name} " "as fraction of target variance"),
                    gen=gen_data[name] / target_data[name],
                )
        return video_data
    ...
    def get_dataset(self) -> xr.Dataset:
        ...
        for label, d in data.items():
            ...
            if d.target is not None:
                ...
                video_data[label] = xr.DataArray(data=d.gen.cpu().numpy(), dims=("timestep", "lat", "lon"))
        return xr.Dataset(video_data)

    def _get_caption(self, name: str) -> str:
        caption = "Autoregressive (left) prediction and (right) target for {name} [{units}]"

...
from src.ace_inference.core.aggregator.inference import InferenceAggregator
...
def test_logs_labels_exist():
    ...
    nz = 3
    loss = 1.0
    area_weights = torch.ones(ny).to(src.ace_inference.get_device())
...
def test_i_time_start_gets_correct_time_longer_windows(window_len: int, n_windows: int):
    ...
    area_weights = torch.ones(4).to(src.ace_inference.get_device())
    nz = 3
    sigma_coordinates = SigmaCoordinates(torch.arange(nz + 1), torch.arange(nz + 1))
    agg = InferenceAggregator(
        area_weights,

...
from src.ace_inference.core.aggregator.plotting import get_cmap_limits, plot_imshow
...
class _TargetGenPair:
    ...
    def weighted_mean_bias(self, weights: torch.Tensor) -> float:
        return float(metrics.weighted_mean_bias(predicted=self.gen, truth=self.target, weights=weights).cpu().numpy())
...
class TimeMeanAggregator:
    ...
    def record_batch(
        ...
        self._target_data = self._add_or_initialize_time_mean(self._target_data, target_data, ignore_initial)
        if self._is_ensemble:
            gen_data = {k: v.mean(dim=0) for k, v in gen_data.items()}  # mean over ensemble members
        self._gen_data = self._add_or_initialize_time_mean(self._gen_data, gen_data, ignore_initial)
    ...
    def get_logs(self, label: str) -> Dict[str, Union[float, torch.Tensor]]:
        ...
        for pred in preds:
            ...
            bias_fig = plot_imshow(bias_data, vmin=vmin_bias, vmax=vmax_bias, cmap="RdBu_r")
            bias_image = wandb.Image(
                bias_fig,
                caption=self._get_caption(bias_map_key, pred.name, vmin_bias, vmax_bias),
            )
                ...
                logs.update(
                    {
                        ...
                        f"bias/{pred.name}": pred.weighted_mean_bias(weights=self._area_weights),
                    }
                )
        logs.update(
            {
                "rmse/channel_mean": sum(rmse_all_channels.values()) / len(rmse_all_channels),
            }
        )
    ...
    def get_dataset(self) -> xr.Dataset:
        ...
        for pred in preds:
            ...
            data.update(
                {
                    f"bias_map-{pred.name}": xr.DataArray(pred.bias().cpu(), dims=dims, attrs=bias_metadata),
                    f"gen_map-{pred.name}": xr.DataArray(
                        pred.gen.cpu(),
                        dims=dims,
                        attrs=gen_metadata,
                    ),
                    f"target_map-{pred.name}": xr.DataArray(
                        pred.target.cpu(),
                        dims=dims,
                        attrs=gen_metadata,
                    ),

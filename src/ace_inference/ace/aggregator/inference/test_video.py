...
def test_video_data(offsets: np.ndarray):
    ...
    for i_start in range(0, len(offsets) - n_window_in_memory + 1):
        ...
        aggregator.record_batch(loss=0, target_data=target_window, gen_data=gen_window, i_time_start=i_start)
    ...
    for i, offset_i in enumerate(offsets):
        np.testing.assert_allclose(data["video/a"].gen[i, :], data["video/a"].target[i, :] + offset_i)
...
def test_video_data_values_on_random_inputs(n_batches: int):
    ...
    assert data["video/min_err/a"].gen.shape[0] == len(offsets)
    assert data["video/max_err/a"].gen.shape[0] == len(offsets)
    np.testing.assert_allclose(data["video/a"].gen.cpu().numpy(), gen["a"].mean(dim=0).cpu().numpy())
    np.testing.assert_allclose(data["video/a"].target.cpu().numpy(), target["a"].mean(dim=0).cpu().numpy())

...
from src.ace_inference.core.aggregator.climate_data import CLIMATE_FIELD_NAME_PREFIXES, ClimateData
...
class DryAir(DerivedMetric):
    ...
    def record(self, target: ClimateData, gen: ClimateData) -> None:
        ...
        if self._dry_air_target_total is None:
            self._dry_air_target_total = torch.zeros_like(dry_air_target, device=self._device)
    ...
    def get(self) -> _TargetGenPair:
        ...
        return _TargetGenPair(target=self._dry_air_target_total, gen=self._dry_air_gen_total)
...
class DerivedMetricsAggregator:
    def __init__(
        ...
        self._derived_metrics: Dict[str, DerivedMetric] = {
            "surface_pressure_due_to_dry_air": DryAir(self.area_weights, self.sigma_coordinates, device=device)
        }
        self._n_batches = 0
    ...
    def record_batch(
        ...
        del target_data_norm, gen_data_norm  # unused

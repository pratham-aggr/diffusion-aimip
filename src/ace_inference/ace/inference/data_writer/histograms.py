...
from src.ace_inference.core.histogram import DynamicHistogram
...
class _HistogramAggregator:
    ...
    def record_batch(
        ...
        if self._target_histograms is None:
            self._target_histograms = {
                var_name: DynamicHistogram(n_times=self._n_times) for var_name in target_data.keys()
            }
        if self._prediction_histograms is None:
            self._prediction_histograms = {
                var_name: DynamicHistogram(n_times=self._n_times) for var_name in prediction_data.keys()
            }
        for var_name, histogram in self._prediction_histograms.items():
            ...
            data = prediction_data[var_name].cpu().numpy().transpose(1, 0, 2, 3).reshape(n_times, -1)
        ...
        for var_name, histogram in self._target_histograms.items():
            ...
            data = target_data[var_name].cpu().numpy().transpose(1, 0, 2, 3).reshape(n_times, -1)
            histogram.add(data, i_time_start=i_time_start)
    ...
    def get_dataset(self) -> xr.Dataset:
        ...
        for missing_target_name in set(prediction_dataset.data_vars) - set(target_dataset.data_vars):
        ...
        for missing_prediction_name in set(target_dataset.data_vars) - set(prediction_dataset.data_vars):
            if not missing_prediction_name.endswith("_bin_edges"):
                ...
                prediction_dataset[f"{missing_prediction_name}_bin_edges"] = target_dataset[
...
class HistogramDataWriter:
    ...
    def flush(self):
        ...
        for name in self.metadata:
            metric_dataset[f"{name}_bin_edges"].attrs["units"] = self.metadata[name].units
        for name in metric_dataset.data_vars:
            if not name.endswith("_bin_edges"):
                metric_dataset[f"{name}_bin_edges"].attrs["long_name"] = f"{name} bin edges"

        ...
        for variable_name in set(target.keys()).union(prediction.keys()):
            ...
            if variable_name not in self.dataset.variables:
                ...
                if variable_name in self.metadata:
                    self.dataset.variables[variable_name].units = self.metadata[variable_name].units
                    self.dataset.variables[variable_name].long_name = self.metadata[variable_name].long_name
            ...
            if variable_name in target:
                ...
                target_numpy = np.full(shape=target[next(iter(target.keys()))].shape, fill_value=np.nan)

...
class TimeCoarsenConfig:
    ...
    def __post_init__(self):
        if self.coarsen_factor < 1:
            raise ValueError(f"coarsen_factor must be 1 or greater, got {self.coarsen_factor}")
...
class TimeCoarsen:
    ...
    def append_batch(
        ...
        if start_timestep == 0:
            ...
            prediction_initial = tensor_dict_time_select(prediction, time_slice=slice(None, 1))
    ...
    def coarsen_batch(
        ...
        batch_times_coarsened = batch_times.coarsen({TIME_DIM_NAME: self._coarsen_factor}).mean()
        return (
            target_coarsened,
            prediction_coarsened,
            start_timestep,
    ...
    def _coarsen_tensor_dict(self, tensor_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:

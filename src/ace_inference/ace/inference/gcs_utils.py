...
def authenticate(keyfile_var: str = KEYFILE_VAR):
    ...
    if dist.is_root():
        try:
            ...
            logging.info("No keyfile found in environment, skipping gcloud authentication.")
        else:
            output = subprocess.check_output(["gcloud", "auth", "activate-service-account", "--key-file", keyfile])

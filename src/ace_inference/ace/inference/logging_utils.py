    ...
    def configure_logging(self, experiment_dir: str, log_filename: str):
        ...
        if self.log_to_file and self._dist.is_root():
            if not os.path.exists(experiment_dir):
                raise ValueError(f"experiment directory {experiment_dir} does not exist, " "cannot log files to it")
    ...
    def configure_wandb(
        ...
        **kwargs,
        ...
        if "environment" in config_copy:
            logging.warning(
                "Not recording environmental variables since 'environment' key is " "already present in config."
            )
...
def retrieve_env_vars(names=ENV_VAR_NAMES) -> Dict[str, str]:
    ...
    for name in names:
        try:
            ...
            pass  # logging.warning(f"Environmental variable {name} not found.")
        else:
            output[name] = value

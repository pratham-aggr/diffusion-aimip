...
from src.ace_inference.core.stepper import SteppedData
...
@register()
def surface_pressure_due_to_dry_air(data: ClimateData, sigma_coordinates: SigmaCoordinates) -> torch.Tensor:
...
@register()
def total_water_path(data: ClimateData, sigma_coordinates: SigmaCoordinates) -> torch.Tensor:
...
@register()
def total_water_path_budget_residual(data: ClimateData, sigma_coordinates: SigmaCoordinates):
    ...
    twp_total_tendency = (total_water_path[:, 1:] - total_water_path[:, :-1]) / (TIMESTEP_SECONDS)
...
def compute_derived_quantities(
    data: Dict[str, torch.Tensor],
    sigma_coordinates: SigmaCoordinates,
    registry: MutableMapping[str, DerivedVariableRegistryEntry] = _DERIVED_VARIABLE_REGISTRY,
) -> Dict[str, torch.Tensor]:
    """Computes all derived quantities from the given data."""

    for label, derived_variable in registry.items():
        data = _compute_derived_variable(data, sigma_coordinates, label, derived_variable)
    return data
...
def compute_stepped_derived_quantities(
    ...
    stepped.gen_data = compute_derived_quantities(stepped.gen_data, sigma_coordinates, registry)
    stepped.target_data = compute_derived_quantities(stepped.target_data, sigma_coordinates, registry)
    return stepped

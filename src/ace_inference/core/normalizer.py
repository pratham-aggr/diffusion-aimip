...
class NormalizationConfig:
    ...
    def __post_init__(self):
        using_path = self.global_means_path is not None and self.global_stds_path is not None
        using_explicit = len(self.means) > 0 and len(self.stds) > 0
        if using_path and using_explicit:
            raise ValueError("Cannot use both global_means_path and global_stds_path " "and explicit means and stds.")
        if not (using_path or using_explicit):
            raise ValueError("Must use either global_means_path and global_stds_path " "or explicit means and stds.")
...
class StandardNormalizer:
    ...
    def from_state(self, state) -> "StandardNormalizer":
        ...
        """
        means = {k: torch.tensor(v, device=get_device(), dtype=torch.float) for k, v in state["means"].items()}
        stds = {k: torch.tensor(v, device=get_device(), dtype=torch.float) for k, v in state["stds"].items()}
        return StandardNormalizer(means=means, stds=stds)
...
def _normalize(
    ...
    return {k: (t - means[k]) / stds[k] if k in means.keys() else t for k, t in tensors.items()}
...
def _denormalize(
    ...
    return {k: t * stds[k] + means[k] if k in means.keys() else t for k, t in tensors.items()}


def get_normalizer(global_means_path, global_stds_path, names: List[str]) -> StandardNormalizer:

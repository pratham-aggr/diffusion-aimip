...
class PrescriberConfig:
    ...
    def __post_init__(self):
        if self.interpolate and self.mask_value != 1:
            raise ValueError("Interpolation requires mask_value to be 1, but it is set to " f"{self.mask_value}.")
...
class Prescriber:
    ...
    def __call__(
        ...
        if self.interpolate:
            ...
            prescribed_gen = mask * target_norm[self.prescribed_name] + (1 - mask) * gen_norm[self.prescribed_name]
        else:
        ...
        gen_norm[self.prescribed_name] = prescribed_gen
        return gen_norm

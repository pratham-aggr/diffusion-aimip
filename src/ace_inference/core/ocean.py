...
class OceanConfig:
    ...
    def build(self, in_names: List[str], out_names: List[str]):
        if not (self.surface_temperature_name in in_names and self.surface_temperature_name in out_names):
            raise ValueError(
...
class Ocean:
    ...
    def __call__(
        ...
        if self.type == "prescribed":
            ...
            next_step_temperature = input_data[self.surface_temperature_name] + temperature_tendency * TIMESTEP_SECONDS
        else:
